...
static char back_stack[8][256]; static int back_sp = 0;
...
static void vkey_submit() {
  ...
  if (vkey_is_search) {
    if (vkey_buf && vkey_buf[0]) {
      char q[256];
      snprintf(q, sizeof q, "http://google.com/search?q=%s", vkey_buf);
      is_search_box = false;
      go_url(q);
  ...
  if (vkey_buf && vkey_buf[0]) go_url(vkey_buf);
  else go_url("mtt:start");
}
...
static const char *PAGE_START =
  ...
  "<p><folder><a href=\"http://google.com/\">Google</a></folder></p>"
  "<p><folder><a href=\"http://m.facebook.com/\">Facebook</a></folder></p>"
  "<p><folder><a href=\"http://en.m.wikipedia.org/\">Wikipedia</a></folder></p>"
  "<p><folder><a href=\"http://dmaxmrp.tk/\">dmaxmrp.tk</a></folder></p>"
  "<p><folder><a href=\"http://m.youtube.com/\">YouTube</a></folder></p>"
...
static const char *PAGE_ABOUT =
  ...
  "<p>Browser V1 English Build101013<br/>Modified by Nervz<br/><br/>"
  "(c) 2013 dmaxmrp.tk<br/>All Rights Reserved.</p></card></wml>";
...
static void push_back() {
  if (back_sp < 8) strncpy(back_stack[back_sp++], cur_url, 255);
}
static void resolve_url(const char *href, char *out, size_t cap) {
  ...
  if (!strncmp(href, "http", 4) || !strncmp(href, "mtt:", 4) || !strncmp(href, "https", 5)) {
    strncpy(out, href, cap - 1); out[cap - 1] = 0; return;
  }
  if (href[0] == '/') snprintf(out, cap, "http://%s%s", last_host, href);
  else {
    char base[256]; strncpy(base, cur_url, sizeof base - 1); base[sizeof base - 1] = 0;
...
static void go_url(const char *url) {
  ...
  if (url && !strcmp(url, "$prev")) {
    if (back_sp) go_url(back_stack[--back_sp]);
    else go_url("mtt:start");
    return;
  }
  ...
  if (!ok || len == 0) { show_msg(ok ? "Server khong tra noi dung" : "Ket noi that bai"); return; }
  // gzip: khong giai nen duoc tren thiet bi -> bao loi
  if (meta.gzipped) { show_msg("Server tra gzip (khong ho tro)"); return; }
  doc.buf = doc_buf; doc.cap = DOC_CAP;
  doc_parse(&doc, net_buf, len);
  history_add(full, doc.title[0] ? doc.title : full);
...
static const MenuTop MENUS[7] = {
  ...
  { "Navg", "Zoom"   },   // page overview
  { "Tool", "Mouse"  },   // virtual mouse
  { "Sett", "WiFi"   },
  { "Help", "Help"   },
  { "Exit", ""       },
};
...
static void render() {
  ...
  disp.setTextFont(2); disp.setTextColor(UI_WHITE);
  disp.drawString(doc.title[0] ? doc.title : "Browser V1.0", UI_PAD + 14, (HDR_H - 8) / 2);
  if (loading) disp.fillRect(0, HDR_H - 3, SCR_W * load_pct / 100, 3, UI_WHITE);

  if (overview_on) {
    // page overview (Opera Mini): moi dong = thanh nho, viewport vien xanh
    int n = doc.nlines > 0 ? doc.nlines : 1;
    int bh = (SCR_H - HDR_H - FTR_H - 8) / (n > 40 ? 40 : n);
    if (bh < 2) bh = 2;
    int max_rows = (SCR_H - HDR_H - FTR_H - 8) / bh;
    for (int i = 0; i < n && i < max_rows; i++) {
      int w = 8 + (doc.lines[i].n % 40) * 3;
      if (w > SCR_W - 30) w = SCR_W - 30;
      disp.fillRect(8, HDR_H + 6 + i * bh, w, bh > 1 ? bh - 1 : 1,
                    doc.lines[i].link >= 0 ? UI_LINK : UI_DIM);
    }
    int vh = max_rows / 4; if (vh < 3) vh = 3;
    int vy = HDR_H + 6 + ov_oy * bh;
    disp.fillRect(4, vy, SCR_W - 8, vh, UI_SEL);
  } else if (vkey_open) {
    vkey_draw();
  } else {
    // body — le deu UI_PAD
    int y = UI_ROWS_Y;
    int focus_y = -1, focus_h = 0;
    for (int i = top; i < doc.nlines && y < SCR_H - FTR_H - 4; i++) {
      int st = doc.lines[i].style;
      int lh = (st == 1) ? 24 : (st == 3) ? 22 : line_h(st);
      if (doc.lines[i].n) {
        const char *txt = doc.buf + doc.lines[i].off;
        bool sel = (i == focus_i);   // Focus Block
        if (sel) { focus_y = y - 2; focus_h = lh + 2; }
        if (st == 1) {
          // field: box deu 2 ben
          ...
          if (sel) disp.fillRect(bx + 2, y + 2, bw - 4, bh - 4, UI_SEL);
          disp.setTextFont(2);
          disp.setTextColor(sel ? UI_SELFG : UI_DIM);
          disp.drawString(txt, bx + 22, y + 7);
        } else if (st == 3) {
          int rh = 22;
          if (sel) disp.fillRect(0, y, SCR_W, rh, UI_SEL);
          else     disp.fillRect(0, y, SCR_W, rh, UI_ROW);
          draw_icon_folder(UI_PAD, y + 5);
          disp.setTextFont(2);
          disp.setTextColor(sel ? UI_SELFG : UI_FG);
          disp.drawString(txt, UI_PAD + 18, y + 7);
          draw_chevron_btn(SCR_W - UI_PAD - 16, y + 3);
        } else {
          if (sel) disp.fillRect(0, y - 2, SCR_W, lh, UI_SEL);
          // font + mau theo style (Opera Mini / bao HTML)
          if (st == 2)      disp.setTextFont(4);          // h1/h2
          else if (st == 5) disp.setTextFont(1);          // small/meta
          else              disp.setTextFont(2);
          uint16_t fg = UI_FG;
          if (doc.lines[i].link >= 0) fg = sel ? UI_SELFG : UI_LINK;
    ...
    // Focus Block: khung vien xanh om khoi dang chon
    if (focus_y >= 0 && focus_h > 0) {
      uint16_t fc = UI_LINK;
      disp.fillRect(0, focus_y, SCR_W, 2, fc);
      disp.fillRect(0, focus_y + focus_h - 2, SCR_W, 2, fc);
      disp.fillRect(0, focus_y, 2, focus_h, fc);
      disp.fillRect(SCR_W - 2, focus_y, 2, focus_h, fc);
    }
    if (mouse_on) {
...
static void jump_link(int dir) {
  // Focus Block: D-Pad di chuyen khung giua cac khoi/dong noi dung
  if (!doc.nlines) return;
  int n = doc.nlines;
  int i = focus_i + dir;
  while (i >= 0 && i < n && doc.lines[i].n == 0) i += dir;
  if (i < 0) i = 0;
  if (i >= n) i = n - 1;
  while (i >= 0 && i < n && doc.lines[i].n == 0) i -= dir;
  if (i < 0) i = focus_i;
  focus_i = i;
  cursor_link = doc.lines[focus_i].link;  // neu khoi co link
  // dua khoi vao khung nhin
  if (focus_i < top) top = focus_i;
  int vis = (SCR_H - HDR_H - FTR_H) / 18;
  if (focus_i >= top + vis) top = focus_i - vis + 1;
  if (top < 0) top = 0;
}
static void on_key(const char *k) {
  ...
  if (overview_on) {
    ...
    if (!strcmp(k, "down")) { if (ov_oy < doc.nlines - 1) ov_oy++; render(); return; }
    if (!strcmp(k, "ok"))   { top = ov_oy; overview_on = false; render(); return; }
    render(); return;
  }
  ...
  if (scr == SCR_INPUT) {
    if (!strcmp(k, "ok")) {           // OK = mo URL / Search
      ...
      if (is_search_box) {
        if (url_in_n) {
          char q[256];
          snprintf(q, sizeof q, "http://google.com/search?q=%s", url_in);
          is_search_box = false;
          go_url(q);
        } else go_url("mtt:start");
      } else {
        if (url_in_n) go_url(url_in); else go_url("mtt:start");
      }
...
extern "C" void url_input_char(char c) {
  ...
  if (c == '\r' || c == '\n') {
    ...
    } else if (url_in_n) go_url(url_in);
    return;
  }
  if (c == 8 || c == 127) {
    mt_slot = -1;
