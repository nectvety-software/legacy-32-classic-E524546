...
bool parseManifest(const char *src,size_t n,Meta &out,const char *&error){
 ...
 else if(!strcmp(out.type,"lua")){if(fields&16){error="Lua app must not declare entry";return false;}}
#endif
 else if(strcmp(out.type,"text")!=0||(fields&16)){error="Unsupported app type or entry";return false;}
 return true;
}
} // namespace Qeapp
