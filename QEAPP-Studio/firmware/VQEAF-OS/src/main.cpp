...
static uint32_t luaPreviousAt = 0;
static uint32_t luaNextFrameAt = 0;
// Lua viewport: status chrome remains untouched; SDK draws in [0,240)×[0,270).
// Time limit is enforced by a count hook inside the VM (no blocking Lua callback).
static void luaRect(void *u,int x,int y,int w,int h,uint16_t color) {
  auto *screen = static_cast<TFT_eSPI*>(u);
  screen->fillRect(x, y+29, w, h, color);
}
static void luaText(void *u,int x,int y,const char *s,uint16_t color) {
  auto *screen = static_cast<TFT_eSPI*>(u);
  screen->setTextColor(color);
  screen->setTextFont(1);
  screen->setCursor(x,y+29);
  screen->print(s);
}
...
static void enterScreen(ScreenId s, bool animate = true, bool resume = false) {
    ...
    if (!appInstaller.get(appCtx.pendingPackageId, meta)) {
      ...
      if (!strcmp(meta.id, "snake_pixel") && !strcmp(meta.type, "text")) {
        ...
        if (systemService.safeMode() || !storage.mounted()) {
          notifications.push("Lua apps", "Disabled in Safe Mode or no SD");
          s = ScreenId::Applications;
        } else {
          String src = appInstaller.installedPath(meta.id) + "/payload.txt";
          File f = storage.fs().open(src, FILE_READ);
          size_t n = f ? size_t(f.size()) : 0;
          char *code = (n>0 && n<=QeLuaRuntime::kMaxSource) ?
              static_cast<char*>(heap_caps_malloc(n+1,MALLOC_CAP_SPIRAM|MALLOC_CAP_8BIT)) : nullptr;
          bool readOk = f && code && f.read(reinterpret_cast<uint8_t*>(code),n)==n;
          if (f)f.close();
          if (readOk)code[n]=0;
          const QeLuaRuntime::Draw draw={luaRect,luaText,luaMillis,&tft};
          bool ok = readOk && luaVm.start(code,n,draw,192*1024);
          if (code)heap_caps_free(code);
          if (!ok) {
            notifications.push("Lua app error",readOk?luaVm.error():"Cannot read signed Lua payload");
            s=ScreenId::Applications;
          } else {
  ...
  switch (s) {
    ...
    case ScreenId::LuaApp:
      ui.chrome(packageOpeningName.length()?packageOpeningName:"Lua app", wifiConnected(),false,false,settings.data().hour12);
      ui.softkeys("", "", "Back");
      if (!luaVm.render()) {
...
#if defined(VQEAF_ENABLE_LUA) && VQEAF_ENABLE_LUA
  if (screen == ScreenId::LuaApp && int32_t(millis()-luaNextFrameAt)>=0) {
    const uint32_t now=millis();
    const float elapsed=(now-luaPreviousAt)/1000.0f;
    luaPreviousAt=now;
    luaNextFrameAt=now+50; // ~20 FPS budget; keep OS input and WiFi responsive.
    if (!luaVm.update(elapsed) || !luaVm.render()) {
      notifications.push("Lua runtime",luaVm.error());
      enterScreen(ScreenId::Applications,false);
    }
  }
  ...
  switch (screen) {
    ...
    case ScreenId::LuaApp:
      if (e.pressed && !e.longPress && e.key==Key::A) next=ScreenId::Applications;
        ...
        const char *key = e.key==Key::Left?"left":e.key==Key::Right?"right":
                          e.key==Key::Up?"up":e.key==Key::Down?"down":
                          e.key==Key::Start?"start":e.key==Key::Option?"option":nullptr;
        if (key && !luaVm.key(key,e.pressed)) {
          notifications.push("Lua runtime",luaVm.error());
