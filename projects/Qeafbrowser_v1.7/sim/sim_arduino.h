...
// Muc dich: chay TRUC TIEP src/*.cpp cua firmware Qeafbrowser tren Windows
...
struct SimWiFi {
  ...
  int last_fail = WL_DISCONNECTED;   // WL_CONNECT_FAILED/WL_NO_SSID_AVAIL sau begin loi
  void mode(int) {}
  void persistent(bool) {}
  void setSleep(bool) {}
  void setAutoReconnect(bool) {}
  void disconnect() { connected = false; SIM_CONNECTED[0] = 0; last_fail = WL_DISCONNECTED; }
  ...
  void begin(const char *ssid, const char *pass = NULL) {
    connected = false;
    last_fail = WL_DISCONNECTED;
    bool known = false;
    for (int i = 0; i < SIM_NET_COUNT; i++) {
      if (strcmp(SIM_NETS[i].ssid, ssid)) continue;
      known = true;
      if (SIM_NETS[i].open || (pass && !strcmp(pass, SIM_EXPECT_PASS))) {
        connected = true;
        strncpy(SIM_CONNECTED, ssid, 63);
        return;
      }
    }
    last_fail = known ? WL_CONNECT_FAILED : WL_NO_SSID_AVAIL;
  }
  int status() { return connected ? WL_CONNECTED : last_fail; }
