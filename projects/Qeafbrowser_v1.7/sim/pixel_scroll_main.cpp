...
static const char *OUTDIR="sim/pixel_scroll_out";
...
static void shot(const char *name){ char p[256]; snprintf(p,sizeof p,"%s/%s.bmp",OUTDIR,name); if(LGFX::inst) LGFX::inst->dumpBmp(p); printf("[shot] %s scroll=%d target=%d active=%d\n",p,sim_body_scroll_px(),sim_body_scroll_target_px(),sim_body_scroll_active()); }
int main(){
#ifdef _WIN32
  ...
  // /long.html: fixture WML dai (khong bat chuot ao, du dai de can cuon pixel).
  ...
  pump(2000); int settled=sim_body_scroll_px(); shot("04_pixel_scroll_settled");
  ...
  pump(2000); int hand_end=sim_body_scroll_px(); shot("09_overview_handoff_settled");
