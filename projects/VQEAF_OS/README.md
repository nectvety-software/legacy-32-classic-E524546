...
Addresses *possible* stack pressure and excessive work when installing signed
`.qeapp` packages. Adds persistent (best-effort RTC) stage markers and Serial
115200 reset diagnostics. Signing, GPIO, `.qeapp` QEAPP/2 and `.vqeaf` themes are
unchanged. A physical ESP32-S3 reset log is required to establish the actual
cause; PC tests cannot prove the reboot is fixed.

- Guide: `docs/INSTALLER_REBOOT_DIAGNOSTIC_V244_VN.md`
- PC check: `python tools/verify_v244.py`
- Device logging: `python tools/capture_install_reset.py --port COM5`
...
### Công cụ thu thập Serial khi cài `.qeapp` (v2.4.4)

Nếu ESP32-S3 khởi động lại trong lúc cài ứng dụng, sử dụng `tools/capture_install_reset.bat`
hoặc `python tools/capture_install_reset.py --port COM5` để ghi log nhận được ở 115200 baud,
đánh dấu Install/RESET và lưu timeline boot. Đọc [hướng dẫn thu log](docs/SERIAL_CAPTURE_INSTALL_RESET_V244.md).
Công cụ PC không sửa hoặc nạp firmware.
