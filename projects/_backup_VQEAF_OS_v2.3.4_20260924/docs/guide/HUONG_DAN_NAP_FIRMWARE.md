...
Log gốc lưu ở [`logs/`](logs/) để đối chiếu. Nhiễu do sandbox của công cụ chạy
(`[safe-delete]`, traceback của `sitecustomize.py`) đã được lọc bỏ — phần còn lại
nguyên văn.
...
| `0x00010000` | `firmware.bin` | 1.473.136 B | Ứng dụng VQEAF OS |

Vùng `app0` rộng 6,25 MiB (`0x640000` byte). `firmware.bin` chiếm 22,48 % vùng đó.
...
- `295 dòng Compiling` + `15 dòng Archiving` (build sạch từ đầu, khoảng **293 giây**).
- `Linking .pio/build/vqeaf_os/firmware.elf`
- `RAM: 26.6% (used 87172 bytes from 327680 bytes)`
- `Flash: 22.5% (used 1472765 bytes from 6553600 bytes)`
- Kết thúc bằng `[SUCCESS]`.

Sinh ra `.pio/build/vqeaf_os/firmware.bin` (1.473.136 B) và `firmware.elf`.
...
Wrote 1473136 bytes (952946 compressed) at 0x00010000 ...
```
...
|---|---:|---|
| `vqeaf_os_v2.3.4_merged.img` | 1.538.672 B | Bản chuẩn, nạp ở `0x0` |
| `vqeaf_os_v2.3.4_16mb.img` | 16.777.216 B | Tool nào bắt buộc ảnh đủ dung lượng chip |

SHA-256:

```
5a96b902f54e5bcd4984e8d74feaa397e072cb148ea4daf8b7f101b9c48c83af  vqeaf_os_v2.3.4_merged.img
e6b75b38d3a5a026374ec41ab788ec4f07c09ba06d276e69dbc1a60f4d615119  vqeaf_os_v2.3.4_16mb.img
```
...
esptool.py --chip esp32s3 --port COM3 --baud 460800 \
  write_flash 0x0 dist/vqeaf_os_v2.3.4_merged.img
```
...
```
Flash will be erased from 0x00000000 to 0x00177fff...
Wrote 1538672 bytes (963323 compressed) at 0x00000000 in 21.6 seconds ...
...
python ~/.platformio/packages/tool-esptoolpy/esptool.py --chip esp32s3 \
  --port COM3 --baud 460800 write_flash 0x0 dist/vqeaf_os_v2.3.4_merged.img
```
...
- `entry 0x403c98d0` — **điểm vào này phải khớp** với `Entry point` mà
  `esptool image_info` báo trên tệp `.img`. Khớp nhau nghĩa là ảnh đúng là
  firmware đang chạy.
...
```
[   493][E][Preferences.cpp:483] getString(): nvs_get_str len fail: themeFile NOT_FOUND
[   501][E][Preferences.cpp:483] getString(): nvs_get_str len fail: note NOT_FOUND
[   513][E][Preferences.cpp:483] getString(): nvs_get_str len fail: last NOT_FOUND
```
...
esptool.py --chip esp32s3 image_info dist/vqeaf_os_v2.3.4_merged.img
