// http.cpp — HTTP/HTTPS client cho ESP_Browser.
// Muc tieu: mot browser nho kieu DMAX/Opera Mini tren ESP32-S3, khong chi UI mock.
...
bool url_normalize_input(const char *input, char *out, size_t cap) {
  ...
  // DMAX cu thuong chap nhan "example.com". Ban moi uu tien HTTPS.
...
static bool perform_request(TClient &client, const char *url, const char *scheme,
  ...
  client.print("User-Agent: Opera/9.80 (J2ME/MIDP; Opera Mini/4.5.33867/191.323; U; vi) Presto/2.12.423 Version/12.16 DMAX-ESP/1.3\r\n");
  ...
  char ck[256]; cookie_get(host, ck, sizeof ck);
...
static bool http_get_once(const char *url, char *buf, size_t cap, size_t *out_len, HttpMeta *meta) {
  char scheme[8], host[64], path[192]; int port = 0;
...
bool http_get(const char *url, char *buf, size_t cap, size_t *out_len, HttpMeta *meta) {
  ...
  *out_len = 0;
  ...
  char cur[256]; strncpy(cur, url, sizeof cur - 1); cur[sizeof cur - 1] = 0;
