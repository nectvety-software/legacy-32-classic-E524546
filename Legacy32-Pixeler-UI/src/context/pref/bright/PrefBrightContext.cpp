...
PrefBrightContext::PrefBrightContext()
{
  ...
  _progress->setBackColor(COLOR_CARD_ALT);
  _progress->setProgressColor(COLOR_ACCENT);
  _progress->setBorderColor(COLOR_PANEL_BACK);
  _progress->setMax(MAX_BRIGHT);
  _progress->setWidth(UI_WIDTH - 5 * 8);
  _progress->setHeight(20);
  _progress->setProgress(_old_bright);
  _progress->setPos((UI_WIDTH - _progress->getWidth()) / 2, 26 + 20);
}
