...
bool FlacDecoder::allocateBuffers(void)
{
  if (!FLACFrameHeader)
  {
    FLACFrameHeader = (FLACFrameHeader_t*)__malloc_heap_psram(sizeof(FLACFrameHeader_t));
  }
  if (!FLACMetadataBlock)
  {
    FLACMetadataBlock = (FLACMetadataBlock_t*)__malloc_heap_psram(sizeof(FLACMetadataBlock_t));
  }
  if (!s_flacStreamTitle)
  {
    s_flacStreamTitle = (char*)__malloc_heap_psram(256);
  }
  ...
  if (psramFound())
  {
    s_samplesBuffer = (int32_t**)ps_malloc(FLAC_MAX_CHANNELS * sizeof(int32_t*));
  ...
  {
    s_samplesBuffer = (int32_t**)malloc(FLAC_MAX_CHANNELS * sizeof(int32_t*));
    for (int32_t i = 0; i < FLAC_MAX_CHANNELS; i++)
    {
...
//----------------------------------------------------------------------------------------------------------------------
void FlacDecoder::setRawBlockParams(uint8_t Chans, uint32_t SampRate, uint8_t BPS, uint32_t tsis, uint32_t AuDaLength)
{
...
//----------------------------------------------------------------------------------------------------------------------
int32_t FlacDecoder::findSyncWord(unsigned char* buf, int32_t nBytes)
{
...
//----------------------------------------------------------------------------------------------------------------------
boolean FlacDecoder::findMagicWord(unsigned char* buf, int32_t nBytes)
{
...
//----------------------------------------------------------------------------------------------------------------------
char* FlacDecoder::getStreamTitle()
{
...
//----------------------------------------------------------------------------------------------------------------------
int32_t FlacDecoder::parseOGG(uint8_t* inbuf, int32_t* bytesLeft)
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
vector<uint32_t> FlacDecoder::getMetadataBlockPicture()
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
int32_t FlacDecoder::parseFirstPacket(uint8_t* inbuf, int16_t nBytes)
{  // 4.2.2. Identification header   https://xiph.org/flac/ogg_mapping.html
  ...
  while (true)
  {
    ...
    switch (bt)
    {
      ...
      case vorbisComment:  // https://www.xiph.org/vorbis/doc/v-comment.html
        ...
        s_flacVendorString = (char*)x_ps_calloc(vendorLength + 1, sizeof(char));
        ...
        for (int32_t i = 0; i < userCommentListLength; i++)
        {
          ...
          {
            vb[0] = x_ps_strndup((const char*)(inbuf + pos + 4 + 6), min((uint32_t)127, commemtStringLength - 6));
            // log_w("TITLE: %s", vb[0]);
          }
          if ((specialIndexOf(inbuf + pos + 4, "ARTIST", 7) == 0) || (specialIndexOf(inbuf + pos + 4, "artist", 7) == 0))
          {
            vb[1] = x_ps_strndup((const char*)(inbuf + pos + 4 + 7), min((uint32_t)127, commemtStringLength - 7));
            // log_w("ARTIST: %s", vb[1]);
          }
          if ((specialIndexOf(inbuf + pos + 4, "GENRE", 6) == 0) || (specialIndexOf(inbuf + pos + 4, "genre", 6) == 0))
          {
            vb[2] = x_ps_strndup((const char*)(inbuf + pos + 4 + 6), min((uint32_t)127, commemtStringLength - 6));
            // log_w("GENRE: %s", vb[2]);
          }
          if ((specialIndexOf(inbuf + pos + 4, "ALBUM", 6) == 0) || (specialIndexOf(inbuf + pos + 4, "album", 6) == 0))
          {
            vb[3] = x_ps_strndup((const char*)(inbuf + pos + 4 + 6), min((uint32_t)127, commemtStringLength - 6));
            // log_w("ALBUM: %s", vb[3]);
          }
          if ((specialIndexOf(inbuf + pos + 4, "COMMENT", 8) == 0) || (specialIndexOf(inbuf + pos + 4, "comment", 8) == 0))
          {
            vb[4] = x_ps_strndup((const char*)(inbuf + pos + 4 + 8), min((uint32_t)127, commemtStringLength - 8));
            // log_w("COMMENT: %s", vb[4]);
          }
          if ((specialIndexOf(inbuf + pos + 4, "DATE", 5) == 0) || (specialIndexOf(inbuf + pos + 4, "date", 5) == 0))
          {
            vb[5] = x_ps_strndup((const char*)(inbuf + pos + 4 + 5), min((uint32_t)127, commemtStringLength - 12));
            // log_w("DATE: %s", vb[5]);
          }
          if ((specialIndexOf(inbuf + pos + 4, "TRACKNUMBER", 12) == 0) || (specialIndexOf(inbuf + pos + 4, "tracknumber", 12) == 0))
          {
            vb[6] = x_ps_strndup((const char*)(inbuf + pos + 4 + 12), min((uint32_t)127, commemtStringLength - 12));
          ...
          if ((specialIndexOf(inbuf + pos + 4, "METADATA_BLOCK_PICTURE", 23) == 0) || (specialIndexOf(inbuf + pos + 4, "metadata_block_picture", 23) == 0))
          {
            ...
            if (s_flacRemainBlockPicLen <= 0)
              s_f_lastMetaDataBlock = true;  // exeption:: goto audiopage after commemt if lastMetaDataFlag is not set
...
//----------------------------------------------------------------------------------------------------------------------
int8_t FlacDecoder::decode(uint8_t* inbuf, int32_t* bytesLeft, int16_t* outbuf)
{  //  MAIN LOOP

  int32_t ret = 0;
  uint32_t segmLen = 0;

  if (s_f_flacFirstCall)
  ...
  if (s_f_oggWrapper)
  {
    ...
    {
      case 0:
        ret = parseFirstPacket(inbuf, segmLen);
        if (ret == segmLen)
        {
          s_flacPageNr = 1;
...
int8_t FlacDecoder::decodeNative(uint8_t* inbuf, int32_t* bytesLeft, int16_t* outbuf)
{
  ...
  while (s_flacStatus == DECODE_FRAME)
...
{
  if (specialIndexOf(inbuf, "OggS", *bytesLeft) == 0)
  ...
  if (FLACMetadataBlock->bitsPerSample < 8)
    return ERR_FLAC_BITS_PER_SAMPLE_UNKNOWN;
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t FlacDecoder::getOutputSamps()
{
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t FlacDecoder::getSampRate()
{
  if (!FLACMetadataBlock)
    return 0;
...
uint32_t FlacDecoder::getAudioFileDuration()
{
  if (getSampRate())
  {                                                             // DIV0
    uint32_t afd = getTotoalSamplesInStream() / getSampRate();  // AudioFileDuration
    return afd;
  }
  return 0;
}
  ...
  else if (8 <= FLACFrameHeader->chanAsgn && FLACFrameHeader->chanAsgn <= 10)
  {
    ...
    if (FLACFrameHeader->chanAsgn == 8)
    {
      for (int32_t i = 0; i < s_numOfOutSamples; i++)
        s_samplesBuffer[1][i] = (s_samplesBuffer[0][i] -
                                 s_samplesBuffer[1][i]);
    }
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
int8_t FlacDecoder::decodeFixedPredictionSubframe(uint8_t predOrder, uint8_t sampleDepth, uint8_t ch, int32_t* bytesLeft)
{  // SUBFRAME_FIXED
...
int8_t FlacDecoder::decodeResiduals(uint8_t warmup, uint8_t ch, int32_t* bytesLeft)
{
  ...
  if (method >= 2)
  {
    return ERR_FLAC_RESERVED_RESIDUAL_CODING;
  }
  ...
  if (s_f_bitReaderError)
    return ERR_FLAC_BITREADER_UNDERFLOW;
  return ERR_FLAC_NONE;
}
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
char* FlacDecoder::x_ps_malloc(uint16_t len)
{
  char* ps_str = NULL;
  if (psramFound())
  {
...
//----------------------------------------------------------------------------------------------------------------------
char* FlacDecoder::x_ps_calloc(uint16_t len, uint8_t size)
{
  char* ps_str = NULL;
  if (psramFound())
  {
...
//----------------------------------------------------------------------------------------------------------------------
char* FlacDecoder::x_ps_strdup(const char* str)
{
  char* ps_str = NULL;
  if (psramFound())
  {
...
//----------------------------------------------------------------------------------------------------------------------
char* FlacDecoder::x_ps_strndup(const char* str, uint16_t n)
{
  char* ps_str = NULL;
  if (psramFound())
  {
