...
public:
  ...
  int32_t parseFirstPacket(uint8_t* inbuf, int16_t nBytes);
  int32_t parseMetaDataBlockHeader(uint8_t* inbuf, int16_t nBytes);
  bool allocateBuffers(void);
  void setDefaults();
  void clearBuffer();
  void freeBuffers();
  void setRawBlockParams(uint8_t Chans, uint32_t SampRate, uint8_t BPS, uint32_t tsis, uint32_t AuDaLength);
  void reset();
  int8_t decode(uint8_t* inbuf, int32_t* bytesLeft, int16_t* outbuf);
  int8_t decodeNative(uint8_t* inbuf, int32_t* bytesLeft, int16_t* outbuf);
  int8_t decodeFrame(uint8_t* inbuf, int32_t* bytesLeft);
  uint32_t getOutputSamps();
  uint64_t getTotoalSamplesInStream();
  uint8_t getBitsPerSample();
  uint8_t getChannels();
  uint32_t getSampRate();
  uint32_t getBitRate();
  uint32_t getAudioDataStart();
...
private:
  ...
  char* x_ps_strndup(const char* str, uint16_t n);
...
private:
  ...
  int32_t** s_samplesBuffer{nullptr};
  FLACFrameHeader_t* FLACFrameHeader{nullptr};
  FLACMetadataBlock_t* FLACMetadataBlock{nullptr};
  uint8_t* s_flacInptr{nullptr};
  char* s_flacStreamTitle{nullptr};
  char* s_flacVendorString{nullptr};
