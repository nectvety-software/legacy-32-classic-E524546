...
static int internal_insert(const char *filename, system_t type)
{
   ...
   case system_nes:
      ...
      /* The PPU renders scanlines 0..239 into bmp->line[scanline], so the
      ** primary buffer must be a full 240 lines tall (NES_VISIBLE_HEIGHT=224
      ** would leave line[224..239] out of bounds -> crash). */
