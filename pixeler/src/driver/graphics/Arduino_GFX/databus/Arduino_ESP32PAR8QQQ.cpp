...
Arduino_ESP32PAR8QQQ::Arduino_ESP32PAR8QQQ(
    ...
    int8_t d6,
    int8_t d7)
    : _dc(dc), _cs(cs), _wr(wr), _rd(rd), _d0(d0), _d1(d1), _d2(d2), _d3(d3), _d4(d4), _d5(d5), _d6(d6), _d7(d7)
{
}
...
void Arduino_ESP32PAR8QQQ::writePixels(const uint16_t* data, uint32_t len)
{
  while (len--)
  {
    _data16.value = *data++;
...
void Arduino_ESP32PAR8QQQ::writeBytes(const uint8_t* data, uint32_t len)
{
  while (len--)
  {
    WRITE(*data++);
