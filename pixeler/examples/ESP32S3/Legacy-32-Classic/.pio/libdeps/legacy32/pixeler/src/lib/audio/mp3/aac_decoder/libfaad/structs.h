...
typedef struct {
    ...
    uint8_t exclude_mask[AAC_MAX_CHANNELS];
    uint8_t additional_excluded_chns[AAC_MAX_CHANNELS];
    real_t  ctrl1;
    real_t  ctrl2;
} drc_info;
typedef struct {
...
typedef struct {
    ...
    uint8_t window_shape_prev[AAC_MAX_CHANNELS];
    uint16_t ltp_lag[AAC_MAX_CHANNELS];
    fb_info*  fb;
    drc_info* drc;
    real_t*   time_out[AAC_MAX_CHANNELS];
    real_t*   fb_intermed[AAC_MAX_CHANNELS];
    ...
    real_t* ssr_overlap[AAC_MAX_CHANNELS];
    real_t* prev_fmd[AAC_MAX_CHANNELS];
    real_t  ipqf_buffer[AAC_MAX_CHANNELS][4][96 / 4];
    pred_state* pred_stat[AAC_MAX_CHANNELS];
    int16_t* lt_pred_stat[AAC_MAX_CHANNELS];
    uint8_t error_state;
    uint32_t __r1; /* RNG states */
    uint32_t __r2;
    uint8_t        pce_set;/* Program Config Element */
    program_config pce;
    uint8_t        element_id[AAC_MAX_CHANNELS];
    uint8_t        internal_channel[AAC_MAX_CHANNELS];
