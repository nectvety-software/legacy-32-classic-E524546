// sim_main.cpp â€” Gia lap chay truc tiep src/*.cpp cua ESP_Browser tren Windows.
...
static void run_tests() {
  ...
  check(file_has("sim_sd/ESPBrowser/config.ini", "wifi_ssid=VNPT-Home"), "config.ini luu SSID");
  check(file_has("sim_sd/ESPBrowser/config.ini", "wifi_pass=abc"), "config.ini luu mat khau");
  ...
  // Focus Block: OK tren khoi co link; dmaxmrp mo qua URL truc tiep
  sim_go_url("http://dmaxmrp.tk/");
  sim_pump(1200); shot("mock_page");
  check(file_has("sim_sd/ESPBrowser/history.txt", "dmaxmrp.tk"), "history luu URL da xem");
...
int main(int argc, char **argv) {
  ::mkdir(g_outdir); ::mkdir("sim_sd"); ::mkdir("sim_sd/ESPBrowser");
  ...
  HWND hwnd = CreateWindow("ESPBsim",
                           "Arduino Firmware Simulator — ESP_Browser (240x320)",
                           WS_OVERLAPPEDWINDOW,
